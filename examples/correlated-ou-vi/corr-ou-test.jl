...
using DataFrames
import LinearAlgebra: PosDefException, Diagonal
using Optim
using BayesScoreCal
...
function testfun(ou::MvNormal, T::Float64, N::Int64, N_importance::Int64, N_samples::Int64, vmultiplier::Float64, alphalevels::AbstractVector{Float64}, checkprobs::AbstractVector{Float64}, iter::Int64; options::Optim.Options = Optim.Options())
    ...
    for alpha in alphalevels
        ...
        tf = EigenAffine(Ndim, 0.1)

        # updates tf
        res = energyscorecalibrate!(tf, cal, w; scaling = M, β = energyβ, options = options, penalty = (10.0, 0.001, 0.001))
    ...
    # Check calibration (no transformation)
    calcheck = coverage(cal, checkprobs)
...
CSV.write("examples/correlated-ou-vi/corr-ou-samples.csv", ssamples)
CSV.write("examples/correlated-ou-vi/corr-ou-covcheck.csv", scalchecks)